    ...
    ajax_jadwal(tahun).then(async function (response) {
      ...
      if (jadwalGanjil.length == 0) {
        ...
        jadwalGanjilBody_HTML = `${jadwalGanjil.map(function (jadwal, i) {
          ...
          <td scope="row">${ucwords(jadwal.kode_prodi)} - ${jadwal.perkuliahan_semester}</td>
          <td scope="row">${ucwords(jadwal.jenis)}</td>
        ...
        <div class="col-md-12">
            <div class="card text-choThem">
                <div class="card-header bg-greenTheme">
                    <h3 class="card-title text-whiteTheme">Tabel Jadwal Perkuliahan Semester <b>Ganjil ${tahunAjaran}</b> <a href="/home/export_excel/ganjil/${tahunAjaranLink}" class="badge bg-maroon ml-1" target="_blank"><i class="far fa-file-excel mr-1"></i>EXPORT</a></h3>
      ...
      if (jadwalGenap.length == 0) {
        ...
        jadwalGenapBody_HTML = `${jadwalGenap.map(function (jadwal, i) {
          return `<tr>
            <td scope="row">${i + 1}</td>
            <td scope="row">${ucwords(jadwal.matkul)}</td>
            <td scope="row">${ucwords(jadwal.kode_prodi)}</td>
            <td scope="row">${ucwords(jadwal.jenis)}</td>
            <td scope="row">${ucwords(jadwal.dosen)}</td>
            <td scope="row">${jadwal.kelas}</td>
            <td scope="row">${jadwal.jumlah_sks}</td>
      ...
      $("#jadwal_genap_wrap").html(`
          ...
          <div class="col-md-12">
              <div class="card text-choThem">
                  <div class="card-header bg-greenTheme">
                      <h3 class="card-title text-whiteTheme">Tabel Jadwal Perkuliahan Semester <b>Genap ${tahunAjaran}</b> <a href="/home/export_excel/genap/${tahunAjaranLink}" class="badge bg-maroon ml-1" target="_blank"><i class="far fa-file-excel mr-1"></i>EXPORT</a></h3>
                  </div>
                  <!-- /.card-header -->
                  <div class="card-body table-responsive p-0">
                    <table class="table table-hover table-bordered text-center">
  ...
  $(document).on('change', '#select-prodi', function () {
    ...
    ajax_kelas(prodi, tahun_ajaran).then(async function (response) {
      ...
      $("#select-matkul").html(`
        ...
        return `<option value='${matkul.kode_matkul}-${matkul.nama_matkul}'>${ucwords(matkul.nama_matkul)} - ${matkul.jenis}</option>`
      }).join("")}
        `);
